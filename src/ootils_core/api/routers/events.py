...
def _build_propagation_engine(db):
    ...
    - 'sql' (DEFAULT since 2026-05-24, cf docs/PERF-BASELINE.md):
        ...
        Measured 5-9× faster than the Python engine on profile S/M.
    - 'python': the in-process Python kernel + dirty-flag pipeline.

    M3 explainability (causal chains) is regenerated **on demand** by
    `GET /v1/explain/{node_id}` (lazy strategy). Neither engine pays the
    eager bulk-generation cost (~5s per 1k shortages on the Python
    builder); that cost is amortized over the few nodes actually
    consulted by agents / UI. See api/routers/explain.py for the lazy
    regen logic.
    """
...
# 002 + 006 + 051 + 062 + 071 + 076 + 079 + 084.
...
VALID_EVENT_TYPES = {
    ...
    "snapshot_captured",
    "outcome_evaluated",
    # From migration 076 CHECK constraint extension (PURGE-1)
    "purge_executed",
    # From migration 079 CHECK constraint extension (ADR-042 PR-3, INT-1 PR3)
    "daily_run_completed",
