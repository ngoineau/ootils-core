...
def _validate_membership(ghost_type: str, members: list[GhostMemberInput]) -> list[str]:
    ...
    if ghost_type == "phase_transition":
        ...
        if member_count > 0:
            errors.append(
                "phase_transition cannot have members with role='member'"
            )
...
async def ingest_ghost(
    ...
    """
    # 1. Validate membership constraints
    if body.members:
    ...
    # 4. Upsert ghost_node — no natural unique key → always insert
    #    (ingest ghosts is a create-or-replace by name+type+scenario for idempotence)
    ...
    if existing_ghost:
        ...
        existing_node_id = existing_ghost["node_id"]
        db.execute(
            """
            UPDATE ghost_nodes
        ...
        ghost_id = uuid4()
        # Create the Ghost node in the graph
        node_id = uuid4()
    ...
    # 5. Upsert members
    if body.members:
        # Delete existing members and re-insert (simplest idempotent strategy)
        db.execute("DELETE FROM ghost_members WHERE ghost_id = %s", (ghost_id,))
        # Also delete existing ghost_member edges
        if node_id:
            db.execute(
        ...
        for m in body.members:
            ...
            if node_id:
                # Find item's node (Item node type)
...
async def list_ghosts(
    ...
    if ghost_type:
        where_clauses.append(f"g.ghost_type = %s")
        params.append(ghost_type)
    if scenario_id:
        where_clauses.append(f"g.scenario_id = %s")
        params.append(scenario_id)
    if ghost_status:
        where_clauses.append(f"g.status = %s")
    ...
    if g["node_id"]:
        ...
        if node_row:
            edges = db.execute(
                ...
                FROM edges WHERE from_node_id = %s AND edge_type = 'ghost_member' AND active = TRUE
                """,
                (g["node_id"],),
            ).fetchall()
            graph_node = {
