...
compares against the linked resource's capacity_per_day × working days per bucket.
...
def run_capacity_aggregate(
    ...
    capacity_per_day = _get_resource_capacity(db, resource_id) if resource_id else 0.0
...
def _get_resource_capacity(db: psycopg.Connection, resource_id: str) -> float:
    """Return capacity_per_day for a resource (with any override on that date in mind)."""
    row = db.execute(
