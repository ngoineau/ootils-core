...
def run_phase_transition(
    ...
    while current <= to_date:
        ...
        proj_a = _get_projected_inventory(db, outgoing["item_id"], scenario_id, current)
        proj_b = _get_projected_inventory(db, incoming["item_id"], scenario_id, current)

        if proj_a is not None and proj_b is not None:
            observed = proj_a + proj_b
            # Baseline: what A would have had alone (ratio = w_out / (w_out + w_in) = w_out)
            # We define baseline as: observed / expected_ratio — simplified:
            # baseline = proj_a / w_out if w_out > 0, else use proj_b
            if w_out > 0:
                baseline = proj_a / w_out
            elif proj_b > 0:
                baseline = proj_b  # at end of transition, B carries everything
            else:
                baseline = 0.0
