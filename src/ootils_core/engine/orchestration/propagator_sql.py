...
Limitations vs. the Python engine:
- Explanation tracking (causal chains) is not regenerated by the SQL
  engine. If `explanation_builder` is configured, only the persistent
  explanation rows from prior calc_runs remain. This is acceptable for
  bulk re-computes / overnight runs; for interactive explainability,
