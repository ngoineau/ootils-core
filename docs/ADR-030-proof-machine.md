...
- **Les 5 statuts :** pour une reco ayant agi avec snapshot présent, les trois derniers statuts sont décidés par `ratio = observé / prédit` en **trois bandes PUREMENT relatives, mutuellement exclusives et exhaustives sur `[0, +∞)`** — **MATERIALIZED testé EN PREMIER** :
  - `MATERIALIZED` — `ratio ≥ MATERIALIZED_FLOOR_RATIO` (la pénurie a eu lieu quand même, y compris un `ratio == 1.0` pleinement matérialisé, **quelle que soit l'échelle absolue de `prédit`**) : `avoided_severity_usd` = `0` (rien de réellement évité — distinct du `NULL` « non calculable »).
  - `AVOIDED` — `ratio ≤ AVOIDED_EPS_RATIO` (déficit observé effectivement nul, résidu de niveau bruit) : `avoided_severity_usd` = le $ prédit (None-honnête si pas de base de coût).
  - `PARTIAL` — `AVOIDED_EPS_RATIO < ratio < MATERIALIZED_FLOOR_RATIO` (réduit, pas éliminé) : `avoided_severity_usd` = la fraction évitée du $ prédit = `prédit × (1 − ratio)`, None-honnête.
  ...
  **Cas dégénéré `prédit` absent/≤0 (pas de ratio formable) :** seul cas où `AVOIDED_EPS_ABS` (plancher absolu) s'applique, comme lecture « bruit sur prédiction inconnue » — `observé ≤ AVOIDED_EPS_ABS` ⇒ `AVOIDED` ; sinon `MATERIALIZED` prudent (rien de prouvé évité). `AVOIDED_EPS_ABS` ne rentre **jamais** dans les bandes de ratio ci-dessus.

  **Correctif adversarial (défaut MAJOR corrigé) :** une version antérieure mélangeait un plancher absolu (`max(prédit × AVOIDED_EPS_RATIO, AVOIDED_EPS_ABS)`) DANS le plafond AVOIDED, testé AVANT MATERIALIZED. Pour tout `prédit ≤ AVOIDED_EPS_ABS / AVOIDED_EPS_RATIO` (= 20 aux défauts), le plancher absolu dominait le ratio et une pénurie **pleinement matérialisée** (`ratio == 1.0`, ex. prédit=1/observé=1) tombait dans `AVOIDED` avec crédit $ complet — `MATERIALIZED` devenait **inatteignable** sur toute la bande `prédit ∈ (0, 20]`. Les bandes sont désormais purement relatives (voir ci-dessus) ; le plancher absolu est confiné au seul cas dégénéré.
- **Seuils 🎯-ajustables pilote, documentés dans l'evaluator** (pas de magie cachée, pas en DB, pas chez le consommateur — classer une observation physique est le travail de l'evaluator) : `AVOIDED_EPS_RATIO` (défaut 0.05), `MATERIALIZED_FLOOR_RATIO` (défaut 0.90), `AVOIDED_EPS_ABS` (défaut 1, plancher absolu — cas dégénéré uniquement). Ils vivent dans `engine/outcome/evaluator.py`.
...
- **`scenario_id` hérité, pas de colonne redondante.** La table ne porte **aucune** colonne `scenario_id` ; le scénario est hérité via `recommendation_id → recommendations.scenario_id`. Les KPI qui slicent par scénario JOIN `recommendations`. Double intention : (1) pas de coordonnée dupliquée sujette à dérive ; (2) **pas de FK vers `scenarios`**, donc la table reste **hors** du garde-fou `test_scenario_fk_retention` (cf. la leçon FK ci-dessous).
  **Correction factuelle (revue adversariale) :** `recommendations.scenario_id` (migration 039) est un `UUID NOT NULL` **sans** `REFERENCES scenarios(scenario_id)` — il n'existe **aucune** FK RESTRICT protégeant la rétention du scénario via la reco. La conception (pas de colonne redondante ici, pas de FK `scenarios` sur `recommendation_outcomes`) reste correcte et `test_scenario_fk_retention` ne s'applique bien pas à cette table — mais pas parce qu'une FK RESTRICT de la reco « protégerait déjà » quoi que ce soit ; aucune protection de ce type n'existe dans ce schéma aujourd'hui. Si la rétention scénario de `recommendations` est un jour resserrée (ajout d'une vraie FK `scenarios`), c'est CETTE migration qui devra porter la discipline RESTRICT — pas la 069.
...
Cinq agrégats SQL, scopés à un scénario, chacun **NULL/0-honnête** — les compteurs `*_basis_count` rendent le dénominateur explicite pour distinguer « aucune donnée » (`NULL`) d'un « zéro » réel :

| # | KPI | Définition | None/0-honnête | Fenêtre `from`/`to` |
|---|---|---|---|---|
| 1 | `pct_shortages_avoided` | `AVOIDED / (AVOIDED+MATERIALIZED+PARTIAL)` sur les recos ACTED | `NULL` si dénominateur 0 (aucun outcome de pénurie à noter) ; `avoided_basis_count` = dénominateur | **Oui** — sur `recommendation_outcomes.evaluated_as_of` |
| 2 | `avoided_severity_usd_total` | `SUM(avoided_severity_usd)` | `NULL` si aucune ligne avoided-$ n'existe (jamais `0` masqué) ; distingué par le compte FILTERé | **Oui** |
| 3 | `avg_fva_wape` | moyenne du **vrai** FVA (`fva_wape` = naive − wape, migration 068) sur les lignes agrégées en scope | `NULL` si aucune ligne WAPE agrégée non-`NULL` ; `fva_basis_count` = dénominateur. Positif = le stat bat la naïve ; négatif légitime, non clampé | **Non** — un run Pyramide n'a pas d'`evaluated_as_of`, scope scénario-vie-entière |
| 4 | `reco_approval_rate` | `APPROVED(+APPLIED) / total recos émises` | `NULL` si aucune reco émise ; `reco_total_count` = dénominateur | **Non** — `recommendations.status` est un état courant sans `evaluated_as_of`, scope scénario-vie-entière |
| 5 | `cost_of_inaction_usd` | `SUM(observed_deficit_qty × unit_cost)` sur les `NOT_APPLICABLE` de recos jamais approuvées ayant matérialisé | `NULL` si aucune telle ligne | **Oui** |

**La fenêtre `from`/`to` est une fenêtre d'OBSERVATION** sur `recommendation_outcomes.evaluated_as_of` — elle ne filtre QUE les KPI 1/2/5 (qui lisent cette table). Les KPI 3 et 4 n'ont pas d'`evaluated_as_of` propre (un run Pyramide, un statut de reco courant) et restent **toujours** scope-scénario-vie-entière, fenêtre ou non — une asymétrie délibérée et documentée (docstring de l'endpoint + du router), jamais un mélange silencieux de données fenêtrées/non-fenêtrées.

Le KPI 5 valorise le déficit matérialisé avec la **même** convention de coût unitaire que l'evaluator pour le côté avoided-$ (`evidence->>'unit_cost'` d'abord **et seulement si `> 0`**, sinon `estimated_cost / recommended_qty`) — une seule convention de valorisation, les deux sens, alignée y compris sur le rejet d'un `unit_cost` négatif ou nul (un défaut MINOR corrigé : la garde SQL initiale n'acceptait le regex numérique que via `NULLIF(...,0)`, laissant passer un `unit_cost` négatif que le côté Python de l'evaluator rejette avec `uc > 0` — même reco, même valeur des deux côtés désormais). Le cast `evidence` est **gardé** par un regex de forme numérique : un `unit_cost` malformé est ignoré (la base `estimated_cost/qty` prend le relais), il n'avorte pas l'agrégat.
...
- **Une colonne `scenario_id` sur `recommendation_outcomes`.** Rejeté : coordonnée dupliquée sujette à dérive, et une éventuelle FK vers `scenarios` sur CETTE table entrerait en conflit avec la règle RESTRICT du garde-fou pendant que l'outcome a besoin d'un CASCADE depuis sa reco. Le scénario est hérité via la reco (dont la propre colonne `scenario_id`, migration 039, n'a aujourd'hui **aucune** FK `scenarios` — voir la correction factuelle en §3 ci-dessus).
...
- `recommendation_outcomes` (migration 069) **n'a pas** de colonne `scenario_id` du tout : elle hérite le scénario via `recommendation_id`, ce qui la garde hors du garde-fou (pas de FK `scenarios` sur cette table → pas de conflit avec le CASCADE dont l'outcome a besoin depuis sa reco). **Précision (revue adversariale) :** ceci ne dépend PAS d'une hypothétique FK RESTRICT sur `recommendations.scenario_id` — cette colonne (migration 039) est un `UUID NOT NULL` sans `REFERENCES scenarios` du tout ; il n'y a donc, aujourd'hui, aucune FK `scenarios` nulle part sur le chemin `recommendation_outcomes → recommendations`, et c'est précisément pour cela qu'aucun conflit RESTRICT/CASCADE ne peut se produire de ce côté.
