...
- `src/ootils_core/db/replica_role.py` — `enable_replica_role` / `restore_origin_role`, la logique SAVEPOINT + fallback partagée (extrait de `manager.py` le 2026-07-12, voir l'extension ci-dessous). `manager.py` conserve des alias module-level (`_enable_replica_role_for_fork`, `_restore_origin_role`) pour ne pas casser le monkeypatch existant dans `tests/integration/test_fork_replica_parity_integration.py`.
...
## Extension 2026-07-12 : second site, les DELETE de PURGE-1

**Contexte mesuré :** la première purge réelle en production (chantier PURGE-1, ADR-039) a pris **2h22 (8 541 s) pour 1,66 M lignes** supprimées à travers les tables de `PURGE_WHITELIST` (`engine/maintenance/purge.py`). Même diagnostic que le fork : les `DELETE` massifs de `apply_fork_purge` déclenchent la validation RI (Referential Integrity) ligne à ligne pour chacune des ~13 FK qui référencent `nodes` (toutes indexées côté colonne référencée — ce n'est donc pas un index manquant, c'est le volume de probes déclenchées par les triggers FK de Postgres, un par ligne supprimée, exactement le mécanisme identifié en §"Contexte" ci-dessus pour l'`INSERT…SELECT` du fork).

**Même argument de sûreté, appliqué au DELETE plutôt qu'à l'INSERT.** `PURGE_WHITELIST` (module docstring de `purge.py`) est construite pour supprimer, dans l'ordre, **toute table qui référence** une table donnée **avant** cette table elle-même (`edges` avant `nodes`, `shortages` avant `nodes`/`calc_runs`/`explanations`, etc. — voir la liste numérotée 1 à 13). Par construction, au moment où le `DELETE` d'une table T s'exécute, plus aucune ligne survivante dans les tables du whitelist ne pointe vers T. La revalidation FK déclenchée ligne-à-ligne par les triggers Postgres sur chaque `DELETE` est donc, comme pour le fork, un travail **redondant** sur un ordre de suppression déjà prouvé sûr — une dérogation de performance, pas de correction.

**Implémentation : le même helper, partagé.** Plutôt que de dupliquer la logique SAVEPOINT + fallback, `_enable_replica_role_for_fork`/`_restore_origin_role` ont été extraites de `manager.py` vers un module neutre, `src/ootils_core/db/replica_role.py` (`enable_replica_role`/`restore_origin_role`, nom de savepoint et libellés de log paramétrables). `manager.py` importe désormais ce module (avec des alias module-level pour préserver le monkeypatch de test existant) ; `engine/maintenance/purge.py`'s `_delete_whitelist_for_scenario` encadre sa boucle de 13 `DELETE` (l'ordre `PURGE_WHITELIST`) avec le même pattern try/finally, son propre nom de savepoint (`purge_delete_replica_role`, distinct de celui du fork pour éviter toute collision si les deux dérogations se retrouvaient un jour imbriquées sur la même connexion) et son propre message de fallback. Comme au site d'origine : seul `psycopg.errors.InsufficientPrivilege` déclenche le repli transparent (un warning, puis le chemin triggers-on inchangé) ; toute autre exception remonte sans être masquée, y compris quand elle survient PENDANT la boucle de `DELETE` — le `finally` qui restaure `session_replication_role = 'origin'` avale uniquement `InFailedSqlTransaction` (transaction déjà avortée par l'exception du `DELETE`), jamais l'exception originale du `DELETE` lui-même.

**Compensation fail-loudly renforcée : un check set-based inconditionnel, sur les deux chemins.** Contrairement au fork (où le check compensatoire porte sur des colonnes FK précises — `item_id`/`location_id`/`projection_series_id`), le check ici est plus direct : `_verify_whitelist_emptied` relit, juste après la boucle de suppression, un `UNION ALL` de `COUNT(*)` scoping `scenario_id` sur les 13 tables de `PURGE_WHITELIST` (réutilisant telles quelles les requêtes de comptage déjà utilisées par `plan_fork_purge`, pour ne jamais diverger d'elles) et lève un `RuntimeError` si le total est `> 0`. Ce check tourne **inconditionnellement**, que la dérogation ait pu s'activer (chemin rapide) ou soit tombée en repli (chemin triggers-on) — la correction ne doit jamais dépendre du chemin emprunté, exactement le principe déjà appliqué au fork (§1). Le coût mesuré est négligeable (~10 ms sur des tables qui viennent d'être vidées). Un `RuntimeError` ici n'est jamais masqué par la suite : `apply_fork_purge`/`_apply_one` ne commitent jamais eux-mêmes (le caller possède la transaction), donc lever avant tout `INSERT` dans `maintenance_purge_runs` ou tout `UPDATE scenarios SET purged_at = …` garantit qu'aucun état partiel n'est jamais committé. `scenarios` (la ligne tombstone, jamais supprimée) et `maintenance_purge_runs` (l'audit row de ce run, pas encore insérée à cet instant) sont explicitement hors de ce check — ce ne sont pas des membres de `PURGE_WHITELIST`.

**Gain attendu :** ~4× sur le temps du `DELETE` loop lui-même (même ordre de grandeur que le 3,4× mesuré côté fork — non re-mesuré ici en production sur le run de 2h22 lui-même, cette extension n'a pas encore été rejouée sur ce volume réel ; le gain reste donc une extrapolation motivée par le même mécanisme sous-jacent, pas une mesure indépendante à cette échelle).

### Code references (extension)

- `src/ootils_core/db/replica_role.py` — le helper partagé (`enable_replica_role`/`restore_origin_role`), les deux sites d'appel documentés dans son propre docstring de module.
- `src/ootils_core/engine/maintenance/purge.py` (`_delete_whitelist_for_scenario`, `_verify_whitelist_emptied`, `_PURGE_DELETE_SAVEPOINT_NAME`, `_VERIFY_WHITELIST_EMPTY_SQL`) — la dérogation côté DELETE et sa compensation set-based.
- `src/ootils_core/engine/scenario/manager.py` — alias module-level (`_enable_replica_role_for_fork`, `_restore_origin_role`) préservant le contrat de monkeypatch de `tests/integration/test_fork_replica_parity_integration.py`.
- `tests/test_purge_replica_role.py` — pins unitaires (ordre SET/DELETE/restore, fallback, non-masquage d'exception) sur le pattern `ScriptedConn` de `tests/test_fork_replica_role.py`.
- `tests/integration/test_purge_integration.py` — lifecycle complet rejoué sur le chemin fallback forcé, et preuve que `_verify_whitelist_emptied` lève réellement sur une ligne résiduelle simulée.
