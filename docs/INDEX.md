...
### Full ADR register (001 → 045, chronological)
...
- [`ADR-043-demand-descent.md`](ADR-043-demand-descent.md) — **Demand descent (DESC-1), Accepted (pilot decision 2026-07-18)**: the pilot's distribution model — demand + safety stock POOLED nationally (planning, Truth B — already summed per item in `mrp/loader.py`), dispatched to real DCs (PAT/DCW/DAL) by state via a per-product % + eligibility (execution, Truth A per-site — already the ADR-021 canonical persistence). No new node_type, zero read-path touched: a dedicated `POST /v1/demand/descend` run materializes derived `ForecastDemand`/`CustomerOrderDemand` on real (`is_stocking=TRUE`) centers, deactivates the national source nodes (anti-double-count, audit kept), and the DRP (merged, previously idle for lack of localized demand — ADR-020) wakes up on its own. Scenario-scoped `demand_split_pct` (migration 083, pattern ADR-025) with baseline fallback; cold-start = flagged equal split; zero-eligible-DC = demand stays national (fail-loudly). `safety_scope=national` default is a decision **acted but not yet coded** — the formal ADR-021 amendment ships in PR-C. Sequencing: PR-F (ERP TSV specs, first) → PR-A (this ADR + migration) → PR-B (the descent run) → PR-C (safety_scope) / PR-D (DRP proof) → PR-E (real % calibration, gated on ERP data).
- [`ADR-044-value-ledger-changed-only.md`](ADR-044-value-ledger-changed-only.md) — **Changed-only ingest + value ledger + decision basis (chantier C2 « moteur d'exception », migration 088), Accepted (2026-07-19)**: the 6 node-upsert arms of `api/routers/ingest.py` re-read business columns and diff via `_changed_fields` — an identical re-push is a STRUCTURAL no-op (no UPDATE / event / `is_dirty` / re-wire), a real change emits ONE typed event PER changed field, finally filling migration-002's long-empty `old_*/new_*` columns (`events` becomes the per-node value ledger — forensic "what value, when, whence" — with no new table). Decision basis: `calc_runs.anchor_date/engine_flavor/code_version` + `recommendations.anchor_date/stream_seq_hwm` make "what the engine knew" a query. Migration-088 discipline = integral superset of 086's CHECK. Keystone of proofs 3 (incremental daily) and 4 (bit-identical replay).
- [`ADR-045-incremental-daily.md`](ADR-045-incremental-daily.md) — **Incremental daily run + weekly full reconciliation (chantier C3 « moteur d'exception »), Accepted (2026-07-19)**: `process_pending(scenario_id, db, *, full=False)` on the base `PropagationEngine` — one polymorphic batch event-consumer (ONE calc_run / advisory lock / `ANALYZE dirty_nodes` #455 / finish, reusing `start_calc_run`'s coalescing + C2 decision basis) that set-based-dirties WHOLE SERIES (the contiguity `PROPAGATE_SQL` needs from bucket 0, invariant (b) `pi_chain_continuity` migration 087, bit-identical to a full per series); `full=True` sweeps the whole scenario. Hard prerequisite C3-PR1 (#489): `resolve_stale` now scoped to series carrying `last_calc_run_id = calc_run_id` (never the whole scenario), full being the limit case (bit-for-bit pre-C3) — this neutralises by construction the old skipped-propagation `shortages` corruption (`POST /v1/calc/run full_recompute=false` resolving every active shortage while re-detecting none). Rewires that non-full POST to `process_pending` (the `full_recompute=true` inline path untouched); versioned `scripts/daily_recompute.py` (incremental by default, `--full`) replaces the VM's unversioned artefact; rust-svc out of scope. No migration in PR2; C3-PR3 = weekly drift (migration 089 `drift_checks` + `drift_detected`).
