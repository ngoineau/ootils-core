...
### SCALE-2 — Décision d'architecture fork — ✅ TRANCHÉ (ADR-041 Accepted, décision pilote 2026-07-13)
**Arbitrage rendu** : la douleur pilote était le **fork gouverné** (watchers/contre-factuels), réglée par ADR-040 (23,8 s → **5,93 s mesurés** sur la pilote, 4×). **Statu quo verrouillé** : `OOTILS_ENGINE` reste `sql`, PyO3-défaut rejeté définitivement (re-bench ±4 %), lazy-CoW différé, sandbox rust-svc **non déclenché**. Réouverture UNIQUEMENT par seuils mesurables (ADR-041 §4) : T1 (what-if interactif p95 > 3 s OU > 20/jour → sandbox B2), T2 (> 5 forks gouvernés/heure + delta shortages canonique → lazy-CoW), T3 (re-bench Rust > 2× SQL → PyO3). Historique de l'item avant arbitrage : le fork ~1 µs (rust/scenario.rs) dort en opt-in ; les deux forks (RAM vs PG) divergent sémantiquement — l'ADR-041 résout la divergence par **attribution de rôle** (PG = canonique/gouverné, RAM = sandbox si T1) plutôt que par unification.
