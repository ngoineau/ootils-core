...
- **Scenario-scoped planning-param overlay (ADR-025, #347):** `scenario_planning_overrides` (migration 060) + the single resolver `resolved_params_sql()` (`engine/scenario/param_overlay.py`) let a fork override 15 whitelisted `item_planning_params` fields (lead times, safety stock, lot sizing — never topology/BOM) without forking master data. PR2 (MRP batch loaders) and PR3 (propagation: `SHORTAGES_SQL`, the propagator's `safety_stock_cache`/`_get_safety_stock`, plus the 5th reader — `mrp.py` simple mode) are merged and wired through this resolver (PR3 via a single-field LATERAL variant, same precedence/SCD2 semantics — no divergent `COALESCE`); PR4 (agent + REST) is not yet wired. `SHORTAGES_SQL` stays scoped persistence, not a truth change — `shortages` ownership is still ShortageDetector-exclusive (ADR-021). Simulation-only: `promote()` never replays overrides onto baseline.
