...
- **Moirai is licence-excluded:** `FM_MOIRAI` (Salesforce Moirai, cc-by-nc-4.0) is commercially excluded (decision locked 2026-05-31) — removed from the Pyramide application enums (`pyramide/models.py`, `engines.py`; API returns the standard unknown-method 422) but tolerated in DB CHECKs for historical rows (migration 057). `FM_CHRONOS` stays supported (stub with AUTO_SELECT fallback).
