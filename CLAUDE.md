...
- **Scenario-scoped planning-param overlay (ADR-025, #347 PR1 foundation):** `scenario_planning_overrides` (migration 060) + the single resolver `resolved_params_sql()` (`engine/scenario/param_overlay.py`) let a fork override 15 whitelisted `item_planning_params` fields (lead times, safety stock, lot sizing — never topology/BOM) without forking master data. Every reader (PR2 MRP loaders, PR3 propagation/`SHORTAGES_SQL`, PR4 agent + REST — not yet wired) MUST compose on this resolver; no new divergent `COALESCE`. Simulation-only: `promote()` never replays overrides onto baseline.
