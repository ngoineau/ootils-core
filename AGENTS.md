...
- Migrations auto-apply on `OotilsDB()` construction (i.e. at API startup), serialized by a PG advisory lock (`_LOCK_KEY = 8_037_421_901`), tracked in `schema_migrations`. Each migration runs in its own transaction (`db/connection.py:181-197`); on **any** error — including "already exists" — the runner logs, rolls back, and re-raises: the migration is NOT recorded as applied and the process crashes at boot. Defensive idempotence (`IF NOT EXISTS` / `CREATE OR REPLACE` / `DROP … IF EXISTS`) is therefore a necessity for clean re-runs after a partial failure — see migration 063's header for the canonical pattern.
